    ...
    <svg width="48" height="68" viewBox="0 0 48 68" fill="none" xmlns="http://www.w3.org/2000/svg"
      ...
      <defs>
        <linearGradient id="rb1" x1="24" y1="2" x2="24" y2="36" gradientUnits="userSpaceOnUse">
          <stop stopColor="white"/><stop offset="0.5" stopColor="#e8d0ff"/><stop offset="1" stopColor="#c170e8"/>
        </linearGradient>
        <linearGradient id="rb2" x1="0" y1="36" x2="0" y2="50" gradientUnits="userSpaceOnUse">
          <stop stopColor="#c170e8"/><stop offset="1" stopColor="#7b2fbe"/>
        </linearGradient>
        <linearGradient id="rb3" x1="24" y1="36" x2="24" y2="46" gradientUnits="userSpaceOnUse">
          <stop stopColor="#d89bf0"/><stop offset="1" stopColor="#c170e8"/>
        </linearGradient>
        <linearGradient id="rf1" x1="0" y1="0" x2="0" y2="1" gradientUnits="objectBoundingBox">
          <stop stopColor="#ff8c00"/><stop offset="0.6" stopColor="#ff4500"/><stop offset="1" stopColor="transparent"/>
        </linearGradient>
        <linearGradient id="rf2" x1="0" y1="0" x2="0" y2="1" gradientUnits="objectBoundingBox">
          <stop stopColor="#fff7aa"/><stop offset="1" stopColor="#ffaa00"/>
        </linearGradient>
      </defs>
    </svg>
  )
        ...
        {isMobile ? (
          ...
          <div
            ...
            onTouchEnd={e => {
              ...
              if (Math.abs(diff) > 50) {
                if (diff > 0) changeStep(Math.min(carouselIdx + 1, steps.length - 1))
                else changeStep(Math.max(carouselIdx - 1, 0))
              }
              touchStartX.current = null
            }}>
            {/* Rocket */}
            <div style={{ textAlign:'center', marginBottom:'20px' }}>
              <div style={{ display:'inline-block', animation: bounce ? 'rocketBounce 0.4s ease' : 'none' }}>
                <Rocket/>
              </div>
            </div>
            {/* Card */}
          ...
          /* DESKTOP ZIGZAG - EXACTAMENTE IGUAL AL ORIGINAL */
          <div style={{ position:'relative', padding:'48px 0' }}>
            <div ref={lineRef} className="proceso-line" style={{ display:'block', position:'absolute', left:'50%', top:0, bottom:0, width:'2px', transform:'translateX(-50%)', background:'linear-gradient(to bottom,transparent,rgba(193,112,232,0.5) 5%,rgba(193,112,232,0.5) 95%,transparent)', zIndex:1 }}/>
            <div style={{ position:'absolute', left:'50%', top:`${rocketY}px`, transform:'translateX(-50%) translateY(-50%)', zIndex:20, pointerEvents:'none' }}>
              <Rocket/>
            </div>
            {steps.map((s, i) => (
              <div key={i} className="proceso-grid" style={{ position:'relative', display:'grid', gridTemplateColumns:'1fr 80px 1fr', alignItems:'center', marginBottom: i < 3 ? '64px' : 0, minHeight:'120px' }}>
                {!s.right ? <SpotlightCard title={s.title} desc={s.desc}/> : <div/>}
                <div className="proceso-center" style={{ display:'flex', alignItems:'center', justifyContent:'center', position:'relative', zIndex:10 }}>
